    ...
    if let Ok(input) = fs::read_to_string(&args[1]) {
        match args[1].split(&['/', '.']).nth(1).unwrap() {
            ...
            "day13" => {
                println!(
                    "Part1: {}\nPart2: {}",
                    advent_of_code_2022::day13::calculate(&input),
                    advent_of_code_2022::day13::calculate_part2(&input)
                );
            }
            "day14" => {
                println!(
                    "Part1: {}\nPart2: {}",
                    advent_of_code_2022::day14::calculate(&input, true),
                    advent_of_code_2022::day14::calculate(&input, false)
                );
            }
            _ => (),
        }
